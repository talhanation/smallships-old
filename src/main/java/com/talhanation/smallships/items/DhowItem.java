...
public class DhowItem extends Item {
    private static final Predicate<Entity> X = EntityPredicates.NO_SPECTATORS.and(Entity::canBeCollidedWith);
    private final DhowEntity.Type type;

    public DhowItem(DhowEntity.Type typeIn, Properties properties) {
        super(properties);
        this.type = typeIn;
    }
    ...
    public ActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if (raytraceresult.getType() == RayTraceResult.Type.MISS) {
            ...
            if (raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
                DhowEntity boatentity = new DhowEntity(worldIn, raytraceresult.getLocation().x, raytraceresult.getLocation().y, raytraceresult.getLocation().z);
