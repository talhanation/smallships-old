...
public abstract class AbstractGalleyEntity extends AbstractBannerUser {
   private final float[] paddlePositions = new float[2];
    ...
    public AbstractGalleyEntity(EntityType<? extends AbstractGalleyEntity> entityType, Level worldIn) {
    ...
    protected void controlBoat() {
        ...
        if (this.isVehicle()) {
            ...
            if (sailstate != 0) {
                switch (sailstate){
                    case 1:
                        f += (0.04F * GalleySpeedFactor * 1/4);
                        if (this.forwardInputDown) {
                            f += (0.02F * GalleySpeedFactor); // speed
                        }
                        break;
                    case 2:
                        f += (0.04F * GalleySpeedFactor * 2/4);
                        if (this.forwardInputDown) {
                            f += (0.02F * GalleySpeedFactor); // speed
                        }
                        break;
                    case 3:
                        f += (0.04F * GalleySpeedFactor * 3/4);
                        if (this.forwardInputDown) {
                            f += (0.02F * GalleySpeedFactor); // speed
                        }
                        break;
                    case 4:
                        f += (0.04F * GalleySpeedFactor * 1);
                        if (this.forwardInputDown) {
                            f += (0.02F * GalleySpeedFactor); // speed
                        }
                        break;
